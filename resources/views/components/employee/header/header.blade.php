<header class="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-4 sm:px-6 z-10 sticky top-0">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-lg font-bold text-slate-900 leading-tight">Welcome back, {{ auth()->user()->name ?? 'Employee' }} 👋</h1>
            <p class="text-xs text-slate-500 hidden sm:block">Employee Workspace Portal</p>
        </div>
    </div>
    ...
    <div class="flex items-center gap-4">
        <!-- Role Badge -->
        <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-md text-xs font-semibold bg-slate-100 text-slate-700 border border-slate-200">
        ...
        <div class="relative" x-data="{ open: false }">
            <button @click="open = !open" class="flex items-center gap-2 text-sm font-medium text-slate-700 hover:text-slate-900 focus:outline-none cursor-pointer">
                <div class="w-8 h-8 rounded-md bg-slate-800 text-white flex items-center justify-center font-bold shadow-xs">
                    {{ strtoupper(substr(auth()->user()->name ?? 'E', 0, 1)) }}
                </div>
                <span class="hidden sm:inline-block font-semibold">{{ auth()->user()->name ?? 'Employee' }}</span>
