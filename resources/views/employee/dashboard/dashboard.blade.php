    ...
    <div class="bg-white border border-slate-200/80 rounded-md p-6 sm:p-8 shadow-xs relative overflow-hidden">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-slate-500 mt-1 text-xs sm:text-sm">Follow your daily shift timeline: Slot 1 Clock-In → 2-Hr Check-In (after 1.5h) → Lunch Break (1h) → 3rd Slot.</p>
            </div>
        ...
        <div class="relative flex flex-col md:flex-row items-center justify-between gap-4 py-2">
            ...
            <div class="hidden md:block absolute top-7 left-10 right-10 h-2 bg-slate-100/90 border border-slate-200/80 rounded-full -z-0 overflow-hidden">
                <div 
                    ...
                    style="width: {{ !$tracking ? '0%' : ($tracking->slot3_end_time ? '100%' : ($tracking->lunch_end_time ? '75%' : ($tracking->lunch_start_time ? '50%' : ($tracking->slot2_checkin_time ? '33%' : '15%')))) }};"
                ></div>
            </div>
            ...
            <div class="flex-1 flex flex-col items-center text-center z-10 w-full md:w-auto">
                @if(!$tracking || !$tracking->lunch_end_time)
                    ...
                    <span class="text-xs font-medium text-slate-400 mt-2">4. 3rd Slot & Clock Out</span>
                    <span class="text-[10px] px-2.5 py-0.5 rounded-full bg-slate-100 text-slate-400 font-medium mt-0.5">Disabled</span>
                ...
                @elseif(!$tracking->slot3_end_time)
                    <div class="w-10 h-10 rounded-full bg-indigo-600 text-white flex items-center justify-center font-bold text-sm ring-4 ring-indigo-100 animate-pulse shadow-md">
                        4
                    </div>
                    <span class="text-xs font-semibold text-slate-900 mt-2">4. 3rd Slot Active</span>
                    <span class="text-[10px] px-2.5 py-0.5 rounded-full bg-indigo-100 text-indigo-900 font-semibold mt-0.5">Started {{ $tracking->slot3_start_time?->format('g:i A') }}</span>

                @else
                    <div class="w-10 h-10 rounded-full flex items-center justify-center font-semibold text-sm {{ $tracking->slot3_is_flagged ? 'bg-red-600 text-white ring-4 ring-red-100' : 'bg-emerald-600 text-white ring-4 ring-emerald-100' }}">
                        @if($tracking->slot3_is_flagged) <i class="ri-error-warning-line text-base"></i> @else <i class="ri-check-line text-base"></i> @endif
                    </div>
                    <span class="text-xs font-semibold text-slate-900 mt-2">4. 3rd Slot Completed</span>
                    <span class="text-[11px] px-2.5 py-0.5 rounded-full {{ $tracking->slot3_is_flagged ? 'bg-red-50 text-red-700 font-semibold' : 'bg-emerald-50 text-emerald-800 font-semibold' }} mt-0.5">
        ...
        @if(!$tracking || !$tracking->slot1_checkin_time)
            <div class="p-5 rounded-md border border-slate-200 bg-slate-50/50 flex flex-col sm:flex-row items-center justify-between gap-4">
                <div>
                    ...
                    <p class="text-slate-500 text-xs mt-0.5">Click below to start your shift. Your Slot 2 check-in will unlock after 1.5 hours.</p>
                </div>
                <button 
                    wire:click="clockIn" 
            ...
            <div class="p-5 rounded-md border border-slate-200 bg-slate-50/50 flex flex-col sm:flex-row items-center justify-between gap-4">
                <div>
                    ...
                    <p class="text-xs text-slate-500 mt-0.5">
                        ...
                        Note: Check-in is allowed between 1.5 hrs and 2.5 hrs. Marking after 2.5 hrs will flag entry.
                    </p>
                </div>
            ...
            <div class="p-5 rounded-md border border-slate-200 bg-slate-50/50 flex flex-col sm:flex-row items-center justify-between gap-4">
                <div>
                    ...
                    <p class="text-xs text-slate-500 mt-0.5">
                        ...
                        Note: Exceeding 3rd slot by <strong>&gt;30 mins</strong> will flag your entry.
                    </p>
                </div>
    ...
    @if($tracking)
        <div class="bg-white border border-slate-200/80 rounded-md p-6 shadow-xs space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                ...
                <div class="p-4 rounded-md border space-y-2 {{ $tracking->slot3_is_flagged ? 'bg-red-50/60 border-red-200' : 'bg-slate-50/80 border-slate-200' }}">
                    <div class="flex items-center justify-between">
                        <span class="font-semibold uppercase tracking-wide text-xs {{ $tracking->slot3_is_flagged ? 'text-red-800' : 'text-slate-800' }}">3rd Slot</span>
                        @if($tracking->slot3_is_flagged)
                            <span class="px-2 py-0.5 rounded-md text-[10px] font-semibold bg-red-600 text-white">
                                Exceeded (+{{ $tracking->slot3_deviation_minutes }}m)
                            </span>
                        @elseif($tracking->slot3_end_time)
                            <span class="text-emerald-700 font-semibold">✓ {{ $tracking->slot3_end_time?->format('g:i A') }} (On Schedule)</span>
                        @else
                            <span class="text-slate-400">Pending</span>
                        @endif
                    </div>
